...
(function(global) {
  ...
  const Utils = {
    ...
    /** Deep clone object - optimized for performance */
    clone(obj) {
      // Handle null/undefined/primitives quickly
      if (obj === null || obj === undefined) return obj;
      if (typeof obj !== 'object') return obj;
      
      // Use structuredClone if available (faster than JSON method)
     ...
     * Escapes dangerous characters in user-provided strings
     */
    escapeHtml(str) {
     ...
     * Alias for escapeHtml for better readability
     */
    sanitize(str) {
    ...
    /**
     * Create safe HTML attribute value
     * Ensures values are properly escaped for use in HTML attributes
     */
    escapeAttr(value) {
  ...
  class StateManager {
    ...
    mergeAvailability(data) {
      const prev = this.state.availability;
    ...
    updateUnit(unitKey, data) {
      const prevUnit = this.state.availability[unitKey];
      // Use shallow copy for better performance
      const next = { ...prevUnit, ...data };
      this.state.availability[unitKey] = next;
      this._notify('availability', this.state.availability, prevUnit);
      this._notify(`availability.${unitKey}`, next, prevUnit);
    }
    ...
    startSync() {
      ...
      ref.once('value')
        .then((snapshot) => {
          ...
          try {
            ...
            Utils.error('FirebaseAdapter', 'Failed to cache data:', e);
          }
      ...
      ref.on('value', (snapshot) => {
        ...
        try {
          ...
          Utils.error('FirebaseAdapter', 'Failed to cache update:', e);
        }
          ...
          .then((snapshot) => {
            ...
            if (data && Object.keys(data).length > 0) {
              ...
              try {
                ...
                Utils.error('FirebaseAdapter', 'Failed to refresh cache:', e);
              }
              this._emit('sync:update', data);
  ...
  class UIManager {
    ...
    _updateAuthUI(user) {
      ...
      if (userInfo) {
        if (user) {
          ...
          const safeEmail = Utils.escapeHtml(user.email || 'User');
          userInfo.innerHTML = `
            <span class="ui-avatar">${initial}</span>
            <span class="ui-name">${safeEmail}</span>
          `;
        } else {
    ...
    _createUnitRow(key, unit) {
      ...
      const updatedBy = unit.updatedBy || 'Unknown';

      // Sanitize all user-provided data to prevent XSS
      const safeName = Utils.escapeHtml(name);
      const safeType = Utils.escapeHtml(type);
      const safeArea = Utils.escapeHtml(area);
      const safeBedrooms = Utils.escapeHtml(bedrooms);
      const safePrice = Utils.escapeHtml(price);
      const safeNotes = Utils.escapeAttr(notes);
      const safeUpdatedBy = Utils.escapeHtml(updatedBy);

      row.innerHTML = `
        <div class="am-unit-header">
          <div class="am-unit-name">${safeName} ${safeType ? '(' + safeType + ')' : ''} ${safeArea ? '• ' + safeArea + 'm²' : ''} ${safeBedrooms ? '• ' + safeBedrooms + 'BR' : ''}</div>
          <div class="am-unit-status am-status-${status}" data-status="${status}">${this._getStatusLabel(status)}</div>
        </div>
        <div class="am-unit-body">
          ${safePrice ? `<div style="font-size:11px;color:var(--pri);font-weight:700;margin-bottom:8px">${safePrice}</div>` : ''}
          ...
          <div class="am-notes">
            ...
            <input type="text" class="am-input" data-action="notes" value="${safeNotes}" placeholder="Add notes...">
          </div>
          ...
          ${unit.updatedAt ? `<div class="am-updated">Updated: ${Utils.formatDate(unit.updatedAt)} by ${safeUpdatedBy}</div>` : ''}
        </div>
      `;
    ...
    showToast(message, type = 'info') {
      ...
      toast.innerHTML = `
        ...
        <span class="am-toast-msg">${safeMessage}</span>
      `;
  ...
  system.saveCache = function() {
    try {
      ...
      Utils.error('AvailabilitySystem', 'Failed to save cache:', e);
    }
  };
  ...
  system.loadCachedData = function() {
    try {
      ...
      Utils.error('AvailabilitySystem', 'Failed to load cached data:', e);
    }
  };
